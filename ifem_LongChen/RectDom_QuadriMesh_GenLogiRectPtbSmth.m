function QuadriMesh = RectDom_QuadriMesh_GenLogiRectPtbSmth(...
...
% and perturbation is smooth (Cf. Wheeler,Xue,Yotov, Numer. Math., 2012, p.193) 
% Input:  
...
% for i=1:(nx+1)
%   for j=1:(ny+1)
%     X(k) = X(k) + 0.06 * sin((i-1)/nx*pi*2) * sin((j-1)/ny*pi*2);
%     Y(k) = Y(k) - 0.05 * sin((i-1)/nx*pi*2) * sin((j-1)/ny*pi*2);
%     k = k + 1;
%   end
% end
