function T = auxstructure_hexa(elem)
%% AUXSTRUCTURE_HEXA auxiliary structure for a 2-D rectangular.
%
%  T = AUXSTRUCTURE_HEXA(elem) constucts the indices map between elements, edges 
...
totalEdge = uint32(sort([elem(:,[2,3]); elem(:,[3,4]); elem(:,[4,1]); elem(:,[1,2])],2));
...
t2 = i2 - NT*(k2-1);
ix = (i1 ~= i2); 
neighbor = uint32(accumarray([[t1(ix),k1(ix)];[t2,k2]],[t2(ix);t1],[NT 4]));% A voir
edge2elem = uint32([t1,t2,k1,k2]);
bdElem = t1(t1 == t2);
bdk1 = k1(t1 == t2);
bdEdge = [elem(bdElem(bdk1==1),[2 3]); elem(bdElem(bdk1==2),[3 4]); elem(bdElem(bdk1==3),[4 1]);...
          elem(bdElem(bdk1==4),[1 2])];
bdEdge2elem = [bdElem(bdk1==1);bdElem(bdk1==2);bdElem(bdk1==3);bdElem(bdk1==4)]; % a voir
