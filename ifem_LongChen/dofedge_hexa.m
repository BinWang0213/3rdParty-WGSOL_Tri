function [elem2edge,edge,elem2edgeSign,edgeSign] = dofedge_hexa(elem)
...
T = auxstructure_hexa(elem);
elem2edge = T.elem2edge;
edge = T.edge;
edge2elem = T.edge2elem;% A voir not correct
...
edgeSign = -ones(NE,1); 
isConsistentEdge = (elem(sub2ind([NT,4],edge2elem(:,1),mod(edge2elem(:,4),4)+1))== edge(:,1)); 
edgeSign(isConsistentEdge) = 1;
%t = edge2elem(e,1); % t is the first triangle containing e
%k = edge2elem(e,4); % the k-th edge of t is e
%elem(edge2elem(:,1) + NT*mod(edge2elem(:,4),4)) % the first node of
...
%% The sign of edges
elem2edgeSign = ones(NT,4);
totalEdge = uint32([elem(:,[2,3]); elem(:,[3,4]); elem(:,[4,1]); elem(:,[1,2])]);
idx = (totalEdge(:,1)>totalEdge(:,2));
elem2edgeSign(idx) = -1;
% isbdEdge = (edge2elem(:,1) == edge2elem(:,2));
% elem2edgeSign(sub2ind([NT,4],edge2elem(:,1),edge2elem(:,3))) = edgeSign;
%  idx = sub2ind([NT,4],edge2elem(~isbdEdge,2),edge2elem(~isbdEdge,3),edge2elem(~isbdEdge,4));
