function [u,AD,b,ers,info] = Stokes_SWG_triangle(node,elem,domain,pde,bdFlag,option)
% This is to solve the Stokes equation on triangular partition.
%
%   u = Stokes_SWG_triangle(node,elem,domain,pde,bdFlag,option) produces the linear
...
%   
%   The function Stokes_SWG_triangle assembes the matrix equation AD*u = b 
...
info.Elem_area=elem_area;
info.Elem_edge_normal=elem_edge_normal;
info.Elem_edge_length=elem_edge_length;
info.NE=NE;
info.erroru=u(1:NE)-uIc(:,1);
info.errorv=u(NE+1:2*NE)-uIc(:,2);
info.errorp=u(2*NE+1:end)-ppI;
...
subplot(2,3,3)
showsolution(node,elem,magntitude-sqrt(uIc(:,1).*uIc(:,1)+uIc(:,2).*uIc(:,2)),[-62,58]); 
title('Speed Error |(u,v)_h|-|(u,v)|');
subplot(2,3,4)
showsolution(node,elem,u(2*NE+1:end),[-62,58]); 
title('Numerical pressure p_h');
subplot(2,3,5)
showsolution(node,elem,pI,[-62,58]); 
title('Exact pressure p');
subplot(2,3,6)
showsolution(node,elem,u(2*NE+1:end)-ppI,[-62,58]); 
...
title('Exact pressure p');
subplot(3,3,9)
scatter3(elem_center(:,1),elem_center(:,2),u(2*NE+1:end)-ppI,30,u(2*NE+1:end)-ppI,'filled'); 
title('Pressure error p_h-p');
...
errorp=h*h*(ppI-u(2*NE+1:end)).*(ppI-u(2*NE+1:end));
...
function [AD,b,u,freeDof,isPureNeumann]= getbdCCFVcoef_triangle(A,b)
    ...
    if ~isempty(bdFlag) % find boundary edges and boundary nodes
        ...
        fixEdge = edge(isDirichlet,:);  % Dirichlet edges  
    ...
    if isnumeric(pde.g_N) && all(pde.g_N == 0)   % zero g_D
        pde.g_N = [];
    end
    % Find Neumann BC edge
    Neumann = [];
    if ~isempty(bdFlag)
