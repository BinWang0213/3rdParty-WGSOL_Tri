...
% 
% Purpose:
%   Compute the element stiffness matrix for SWG applied to Stokes
%   equation -Delta u + \nabla p = (f1, f2), 
%             div (u) = f3
...
%visc=1.0; % viscosity will be set later in the code with information from pde. 
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
% Number of edges % should be an input variable to the function
%
num_edge = MyElement.NumberOfEdge;
%
% coordinates of nodal points, already passed in through the function input
%
...
Polygon_Area=area;
%
% Obtain viscosity at the center of the element
%
visc=pde.viscosity(center);
%
...
% Q_1 matrix
%
 Q_One = EdgeMatrix*unitnormal_vec(:,1);
 Q_Two = EdgeMatrix*unitnormal_vec(:,2);
%
% Get element stiffness matrix
%
h=sqrt(area);
T_M = kappa*h^alpha*A_Matrix+B_Matrix;
Ele_Stiffness = [T_M, zeros(num_edge,num_edge), -Q_One; ...
                 zeros(num_edge,num_edge), T_M, -Q_Two; Q_One', Q_Two', 0];
%
...
% polynomial function
%
%C poly_xx=@(x,y) (x-center(1))^2;
%C poly_xy=@(x,y) (x-center(1))*(y-center(2));
...
coef12=0.0;
%
% The following computation is not needed because xc has been computed as
% the center of the element. This part needs to be turned on if xc is not
% the element center
%C for ii=1:num_edge
%C    ii_left=edge_info(ii,1);
%C    ii_right=edge_info(ii,2);
%C   fleft=poly_xx(nodal(ii_left,1), nodal(ii_left,2))/2;
%C   fright=poly_xx(nodal(ii_right,1), nodal(ii_right,2))/2;
%C   fmid=poly_xx(edge_mid(ii,1), edge_mid(ii,2))/2;
%C   coef12=coef12+mag_edge(ii)*(fleft+4.0*fmid+fright)*unitnormal_vec(ii,1)/6;
%Cend
%
% For \int_T (y-yc) dT
%
coef13=0.0;
%
% The following computation is not needed because yc has been computed as
% the center of the element. This part needs to be turned on if yc is not
% the element center
%C for ii=1:num_edge
%C    ii_left=edge_info(ii,1);
%C    ii_right=edge_info(ii,2);
%C   fleft=poly_xy(nodal(ii_left,1), nodal(ii_left,2));
%C   fright=poly_xy(nodal(ii_right,1), nodal(ii_right,2));
%C   fmid=poly_xy(edge_mid(ii,1), edge_mid(ii,2));
%C   coef13=coef13+mag_edge(ii)*(fleft+4.0*fmid+fright)*unitnormal_vec(ii,1)/6;
%C end
%
%
% For \int_T (x-xc)^2 dT
...
%
% The following is the load vector valued function \bf f: for test purpose.
% This part of the code is not needed in practical use.
%
...
%
% The following is for designing test case
%f_two_vec=[rhsf_two(xc,yc), rhsf_two_x(xc, yc), rhsf_two_y(xc,yc)];
%
f_two_vec=[ff(:,2)', DF(:,2,1)', DF(:,2,2)'];
load_Ftwo=f_two_vec*coef*D_Matrix;
%
% For the mass conservation equation.
%
f_three_vec=[ff(:,3)', DF(:,3,1)', DF(:,3,2)'];
%
coefthree=[area; coef12; coef13];
load_Fthree=f_three_vec*coefthree;
% The following is the element load vector
%
Ele_Load=[load_Fone';load_Ftwo';load_Fthree];
%
end
