...
for k = 1:maxIt
    ...
    case 'triangle'
     ...
     %Mesh generator from DarcyLite
     %{
     NXY=2/h(k);
     TriMesh = RectDom_TriMesh_GenUnfm(domain(1),domain(2),NXY,domain(3),domain(4),NXY,2);
     node=TriMesh.node;
     elem=TriMesh.elem;
     %}
     
     %Mesh generator from Gmsh
     %[msh,node,elem]=Gmsh_Read('simple.msh');
     
     %Swicth 1st element to the center
     %elem([size(elem,1)/4-sqrt(size(elem,1)/2)/2 1],:) = ...
     %  elem([1 size(elem,1)/4-sqrt(size(elem,1)/2)/2],:);
     ...
     figure('Name',' L^2 for RT0 velocity (u,v)_h '); 
     showrate(H, ERR_triangle(:,4),1,'-*','L^2 for RT0 velocity (u,v)_h ');
     hold off;
         end
    ...
    %Long chen mehs generator
    %[nodeq,elemq] = squarequadmesh(domain,h(k));
    
    %James Liu mesh generator
    %%{
    delta = 1.0;  status = 2;
    NXY=2/h(k);
    QuadriMesh = RectDom_QuadriMesh_GenLogiRectPtbSmth(...
    0,2,NXY,0,2,NXY,delta,status);
    %QuadriMesh = RectDom_QuadriMesh_GenLogiRectPtbRand(...
    %0,2,NXY,0,2,NXY,delta,status);
    nodeq=QuadriMesh.node;
    elemq=fliplr(QuadriMesh.elem); %requires Clock-wise order
    %%}
    figure(2);
    showmesh(nodeq,elemq);
    findnode(nodeq);
    findelem(nodeq,elemq);
    hold off;
